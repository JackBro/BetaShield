...
typedef FARPROC(WINAPI* lp_GetProcAddress)(_In_ HMODULE hModule, _In_ LPCSTR  lpProcName);
...
extern WINAPI_MODULE_TABLE* BetaModuleTable;
...
typedef struct _WINAPI_API_TABLE
{
	...
	lp_GetProcAddress _GetProcAddress;
	lpGetModuleHandleA GetModuleHandleA;
	lpGetModuleHandleA _GetModuleHandle;
	...
	lpProcess32First Process32First;
	lpProcess32Next Process32Next;
	lpModule32First Module32First;
	lpModule32Next Module32Next;
	...
	lpZwQuerySystemInformation ZwQuerySystemInformation;
	...
	lpReadFile ReadFile;
	lpLdrGetDllFullName LdrGetDllFullName;
	...
	lpEvtFormatMessage EvtFormatMessage;
	lpEvtClose EvtClose;
	lpGlobalFree GlobalFree;
}WINAPI_API_TABLE, *PWINAPI_API_TABLE;
extern WINAPI_API_TABLE* BetaFunctionTable;
#pragma optimize("", on )
...
class CWinapi {
	...
	public:
		const char*		GetPythonName() { return LPData->GetPythonName().empty() ? "" : LPData->GetPythonName().c_str(); };
		DWORD			LastError()		{ return LPData->DynamicAPIsIsInitialized() ? BetaFunctionTable->GetLastError() : GetLastError(); };

		PVOID			GetModuleAddressFromName(const wchar_t* c_wszName, bool bIsCompleteCheck = true);
		bool			IsLoadedAddress(DWORD dwAddress);
		bool			DestroyEntrypoint(DWORD dwAddress);

		void			InitDynamicWinapis();
};
extern CWinapi* LPWinapi;
